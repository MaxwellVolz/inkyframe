...
def parse_tide_table(tide_soup):
    forecast_table = tide_soup.select_one(
        "#contdiv > div.tide-table > div.has-inertia.tide-table__container > div > div > table > tbody"
    )
    ...
    # Initialize table
    t = PrettyTable(
        ["Date", "Day", "Time", "Wave Height", "Wave Rating", "Wind MPH", "Wind Dir"]
    )
    ...
    low_tides = forecast_table.find_all(
        "td", class_="tide-table__part tide-table__part--low tide-table__part--tide"
    )[:4]

    # Find all td elements that have all these three classes

    high_tides = forecast_table.find_all(
        "td",
        class_=lambda x: x
        and "tide-table__part" in x
        and "tide-table__part--high" in x
        and "tide-table__part--tide" in x,
    )[:4]
    low_tides = forecast_table.find_all(
        "td",
        class_=lambda x: x
        and "tide-table__part" in x
        and "tide-table__part--low" in x
        and "tide-table__part--tide" in x,
    )[:4]
